...
class AutoCollector(Protocol):
    ...
    def collect(self, update: Any) -> Optional[str]:
...
class UsernameAutoCollector:
    ...
    def collect(self, update: Any) -> Optional[str]:
        ...
        if hasattr(update, 'message') and update.message and hasattr(update.message, 'from_user') and update.message.from_user:
...
def auto_collect_username(update: Any) -> Optional[str]:
    return AutoCollectorFactory.create_username().collect(update)

def auto_collect_first_name(update: Any) -> Optional[str]:
    return AutoCollectorFactory.create_first_name().collect(update)

def auto_collect_full_name(update: Any) -> Optional[str]:
