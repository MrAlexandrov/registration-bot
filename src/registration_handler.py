...
class RegistrationFlow:
    def __init__(self, user_storage):
        ...
        self.post_registration_states = {state["name"]: state for state in POST_REGISTRATION_STATES}
    ...
    async def transition_state(self, update, context, state):
        ...
        if state in self.post_registration_states:
            await self.handle_post_registration_state(update, context, state)
        else:
            await self.handle_field_state(update, context, state)

    async def handle_field_state(self, update, context, state):
        """Обрабатывает состояния, связанные с вводом данных (регистрация и редактирование)."""
        user_id = update.message.from_user.id
        ...
        print(f"[DEBUG] Запрос '{field_config['message']}' у пользователя {user_id}")
    ...
    async def handle_post_registration_state(self, update, context, state):
        ...
        user_id = update.message.from_user.id
        config = self.post_registration_states[state]
    ...
    async def handle_input(self, update, context):
        ...
        if current_state.startswith("edit_") or current_state in self.steps:
            await self.process_field_input(update, context, current_state)
        elif current_state in self.post_registration_states:
            await self.process_post_registration_input(update, context, current_state)
        else:
    ...
    async def process_post_registration_input(self, update, context, current_state):
        """Обрабатывает ввод в состояниях 'registered' и 'edit'."""
        user_id = update.message.from_user.id
        user_input = update.message.contact.phone_number if update.message.contact else update.message.text
        ...
        field_config = next((f for f in FIELDS if f["label"] == user_input), None)
