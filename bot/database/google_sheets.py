...
# Реализация хранения в Google Sheets
class GoogleSheetsStorage(Storage):
    def __init__(self, credentials_file: str, spreadsheet_id: str, sheet_title: str, debug_mode: bool = False):
        self.client = GoogleSpreadsheetClient(credentials_file, debug_mode=debug_mode)
        self.client.set_spreadsheet(spreadsheet_id)
        self.client.set_sheet_by_title(sheet_title)
        self.sheet_title = sheet_title
        logger.info("Инициализирован GoogleSheetsStorage.")

    def add_user(self, user: User) -> None:
        # Предполагаем, что данные начинаются с первой строки (без заголовков)
        values = [[user.telegram_nick, user.last_name, user.first_name, user.patronymic, user.group, user.birth_date]]
        self.client.prepare_set_values('A2', values, major_dimension="ROWS")
        self.client.execute_batch_update()
        logger.debug(f"Добавлен пользователь в Google Sheets: {user}")

    def update_user(self, user: User) -> None:
        # Получаем все пользователи
        users = self.get_all_users()
        for idx, existing_user in enumerate(users, start=2):  # Предполагаем, что данные начинаются с 2 строки
            if existing_user.telegram_nick == user.telegram_nick:
                start_cell = f"A{idx}"
                values = [[user.telegram_nick, user.last_name, user.first_name, user.patronymic, user.group, user.birth_date]]
                self.client.prepare_set_values(start_cell, values, major_dimension="ROWS")
                self.client.execute_batch_update()
                logger.debug(f"Обновлён пользователь в Google Sheets: {user}")
                break

    def get_all_users(self) -> List[User]:
        values = self.client.get_values_by_range('A2:F')  # Предполагаем, что заголовки в первой строке
        users = [
            User(
                telegram_nick=row[0],
                last_name=row[1],
        ...
        logger.debug(f"Получено {len(users)} пользователей из Google Sheets")
        return users
